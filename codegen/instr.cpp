...
/////////////////////////////
tuple4_vec UnOp::genCode(context c){
    std::string hook;
    if(c.hook_addr.empty())
        hook = genAddr();
    else
    ...
    std::string addr1 = genAddr();
    context c_child = context(addr1, c.break_label, c.continue_label);
    tuple4_vec child_v = child->genCode(c_child);
    
    tuple4 op(this->instr, hook, addr1, "");    // "" correct?
    child_v.push_back(op);
    return child_v;
}
